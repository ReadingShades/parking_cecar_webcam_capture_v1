...
with gr.Blocks(css=css) as demo:
    ...
    with gr.Row():
        ...
        live_vehicle_detection_cam = gr.Image(shape=(640, 480), interactive=False, show_label=False)
    ...
    with gr.Row():
        with gr.Column():
            ...
            license_detection_img = gr.Image(shape=(640, 480), interactive=False, show_label=False)
        with gr.Column():
            gr.Markdown("### license_crop")
            license_detection_crop = gr.Image(shape=(640, 480), interactive=False, show_label=False)
            ocr_output = gr.TextArea(lines=4, max_lines=10, interactive=False, label="OCR detections")
        with gr.Column():
